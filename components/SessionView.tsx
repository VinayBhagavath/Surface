...
// Driven by useEvidenceRun; live mode and fixture mode share the same state shape.
//
...
export function SessionView({
  ...
  const displayContext = clinicalContext?.trim() || meta.clinicalContext;
  const isExactDemo = displayVariant === meta.variant;
  const displayGene = isExactDemo ? meta.gene : "Live run";
  const displayScenario = isExactDemo ? meta.scenario : "Live evidence path";
  const displayTone = isExactDemo ? meta.tone : "pending";
  ...
  function buildContext(): FollowupContext {
    ...
    return {
      geneSymbol: displayGene,
      variant: displayVariant,
      clinicalContext: displayContext,
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            <span className="font-mono text-[0.65rem] uppercase tracking-wider text-muted-foreground">
              {displayContext.replace(/_/g, " ")} · run {runId.slice(0, 8)}
            </span>
          </div>
        </div>
        <div className="flex items-center gap-2 sm:gap-3">
          <span className="hidden items-center gap-1.5 md:flex">
            <span className={cn("size-2 rounded-full", toneDot(displayTone))} />
            <span className="text-xs text-muted-foreground">{displayScenario}</span>
          </span>
          {ttsSupported && (
            <Button
